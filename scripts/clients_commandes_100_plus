...
def fetch_clients_with_large_orders(conn):
    """
    Récupère les clients ayant passé des commandes de plus de 100 euros depuis la base de données.
    
    Paramètres :
    conn (sqlite3.Connection) : Objet de connexion à la base de données SQLite.
    
    Retourne :
    pandas.DataFrame : DataFrame contenant les clients ayant passé des commandes de plus de 100 euros.
    """
    query = '''
...
def main():
    ...
    csv_output_path_orders = '../csv/clients_commandes_plus_100.csv'
    
    try:
        # Étape 1 : Se connecter à la base de données
        with connect_to_database(db_path) as conn:
            # Étape 2 : Récupérer les clients ayant passé des commandes de plus de 100 euros
            large_orders_clients_df = fetch_clients_with_large_orders(conn)
            
            # Étape 3 : Afficher les résultats
            if not large_orders_clients_df.empty:
                print("Clients ayant passé des commandes de plus de 100 euros :")
                print(large_orders_clients_df)
            else:
                print("Aucun client n'a passé de commande de plus de 100 euros.")
            
            # Étape 4 : Exporter les résultats vers un fichier CSV
            export_to_csv(large_orders_clients_df, csv_output_path_orders)
